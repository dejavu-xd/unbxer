let stage = 0;
let skillData = [];
let { str, config } = JSON.parse(sessionStorage.getItem('data'));

(function dartProgram()
{function copyProperties(a,b){var s=Object.keys(a)
...
tP(a){H.cU(new P.kC(a),0)()},
tQ(a){H.cU(new P.kD(a),0)()},
...
// console.log(p.ch, config[stage].count, p.Q * 1e4 / p.ch, wl.join(''));
// wl = [];
if (p.ch >= config[stage].count) {
  const { score, skill } = config[stage];
  const curScore = round(p.Q * 1e4 / p.ch);
  if (skill)
    skillData = Object.values(p.z.b).map(x => {
      var t = x.a.match(/\[([^\d]+)\]/)?.[1] ?? '零伤';
      return [t, round(x.b * 1e2 / p.ch)]
      // return [x.a, round(x.b * 1e2 / r.ch)]
    });
  if (curScore < score) {
    resolve(false, skillData, curScore, p.ch);
    throw '';
  } else if (stage === config.length - 1) {
    resolve(true, skillData, curScore, p.ch);
    throw '';
  }
  stage++;
}
